...
public class ListGenericRun {
	 ...
	 * 	  매번 다형성에 의해 " 형변환" 하는 절차를  줄이기 위해(코드가 깔끔)
	 * 
	 * 
	 * *재네릭 설정 시 주의 사항
...
//		>JDK 7버전 부터 뒤쪽의 제네릭 타입은 명시 안해도 됨 !!
...
//		1.add(E e)
		...
		list.add(new Music("How Sweet","뉴진스"));
		list.add(new Music("클락션", "여자아이들"));
//		list.add("끝")  // 불가 Music 타입만 들어갈수 있다.
		
		System.out.println(list);
//		>이제는 해당 Music 타입만 취급할 수 있다.
		
//		2.add(int index, Music e)
...
//		6.get(ing index) : E (Music 설정
//		제네리 설정 전(다형성을  적용해야함(형변환 해줘야함)
...
//		ArrayList<int> intList = new ArrayList<>();//기본자료형 불가
		ArrayList<Integer> intList = new ArrayList<>(); // 참조자료형 가능
