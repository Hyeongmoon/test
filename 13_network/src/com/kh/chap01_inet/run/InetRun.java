...
public class InetRun {
	 ...
	 * 여러대의 컴퓨터들이 서로 연결되어 있는 통신망을 일컫는 용어
	 * 네트워크를 통해 서로간의 데이터를 주고받을 수 있다. 
	 * 
	 * * 'IP 주소' 와 'Port 번호'
	 * - IP주소 : 네트워크 상의 각 컴퓨터들을 "식별" 해줄 수 있는 번호(주소)
	 * 			 즉, 컴퓨터의 주민번호 같은 개념
	 * 
	 * -Port 번호 : 컴퓨터 안에서 작동하는 여러개의 프로그램 중에 하나를 찾을 때, 
	 * 			   각각의 프로그램들을 "식별"해주는 내부 주소 번호
	 * 
	 * >IP주소, Port 번호는 고정값이 아닌 변동이 가능한 유동적인 값이다.
	 * 	단, 값을 바꿀 때 IP 주소 같은 경우 같은 네트워크 범위 안에서, 
	...
	public static void main(String[] args) {
		...
		try {
			...
			//내 pc명과 IP 주소 쪼개기
			//> 내가 직접 알아낼 수 있지만, 내 PC명, 내 IP 주소를 바로 얻어낼 수 있다.
			...
			// > 도메인 주소 : 사용자가 웹사이트에 접속할때
			//				원칙 상 IP 주소와 Port 번호를 대고 접속하는게 맞지만
			//				매번 그 IP 주소를 외우기 어렵기 때문에 
			//  			외우기 쉬운 구조로 사용자에게 제공해주는 주소
			
			//>주소창에 도메인 주소를 쳤을때 일어나는 일
			// 도메인 주소 입력 --> DNS 서버 --> 해당 IP주소로 요청을 보내줌
			
			// DNS(Domain Name Server) : 도메인 주소 + IP 주소들이
			//  						 1:1 로 맵핑되어있는 서버
			//예) 172.217.31.4 - www.google.com
			//	 233.130.192.247 - www.naver.com
			//	 
			...
			//> 구글의 서버는 여러대임 !! 분산 구조로 이루어져 있다.
			//	요청이 한곳에만 몰빵될 경우 해당 서버에 과부하가 걸리기 때문 !!
			// 예) 수강신청, 티켓팅 등... => DDos 공격의 원리임
			
			System.out.println("---------------------------");
			
			//도메인 주소를 통해서 그 서버"들" 에 대한 정보를 한번에 얻어내기보기
			//> 그 서버들의 정보들이 배여로 돌아올것
			...
			for(int i = 0 ; i<naverHost.length;i++) {
				//System.out.println(naverHost[i]);// 서버명/IP 주소
				System.out.println(naverHost[i].getHostAddress()); //IP 주소만.
			}
			*/
 ...
 *  
 *  *자바만을 가지고 서버와 클라이어트 간의 간단한 통신 해보기
